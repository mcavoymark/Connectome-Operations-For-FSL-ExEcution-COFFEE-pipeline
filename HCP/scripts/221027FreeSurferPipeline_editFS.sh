...
echo "$0 freesurferVersion=$freesurferVersion"
#if [[ "${freesurferVersion}" != "5.3HCP" && "${freesurferVersion}" != "7.2" ]];then
#START220917
#if [[ "${freesurferVersion}" != "5.3HCP" && "${freesurferVersion}" != "7" ]];then
#    echo "$0 Unknown version of freesurfer ... ${freesurferVersion}"
#    exit
#fi
#START221027
if [[ "${freesurferVersion}" != "5.3.0-HCP" && "${freesurferVersion}" != "7.2.0" && "${freesurferVersion}" != "7.3.2" ]];then
    echo "$0 Unknown version of freesurfer. freesurferVersion=${freesurferVersion}"
    exit
fi
...
if [ "${editFS}" != "TRUE" ];then
    ...
    if [[ "${startHiresWhite}" != "TRUE" && "${startHiresPial}" != "TRUE" ]];then
        ...
        if [ "${singlereconall}" = "TRUE" ];then
            ...
            #recon-all -all -i "$T1wImageFile"_1mm.nii.gz -subjid $SubjectID -sd $SubjectDIR -openmp ${num_cores} ${seed_cmd_appendix}
            #START220910
            #if [ "${freesurferVersion}" = "5.3HCP" ];then
            #    recon-all -all -i "$T1wImage" -subjid $SubjectID -sd $SubjectDIR -openmp ${num_cores} ${seed_cmd_appendix}
            #elif [ "${freesurferVersion}" = "7.2" ];then
            #    recon-all -all -i "$T1wImage" -subjid $SubjectID -sd $SubjectDIR -T2 "$T2wImage" -T2pial -openmp ${num_cores} ${seed_cmd_appendix}
            #fi
            #START220913
            #if [ "${freesurferVersion}" = "5.3HCP" ];then
        ...
        #START220913
            ...
            #fi
            #START221027
            if [[ "${freesurferVersion}" = "7.2.0" ]];then
                if [ -n "$T2wImage" ];then #220806FreeSurferHiresWhite.sh
                    recon-all -subjid $SubjectID -sd $SubjectDIR -autorecon2 -autorecon3 -T2 "$T2wImage" -T2pial -openmp ${num_cores} ${seed_cmd_appendix}
                else
                    recon-all -subjid $SubjectID -sd $SubjectDIR -autorecon2 -autorecon3 -openmp ${num_cores} ${seed_cmd_appendix}
